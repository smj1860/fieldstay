 ...
 * Returns all personal data held for the authenticated user as a JSON attachment.
 */
...
export async function GET() {
  ...
  const [
    ...
    supabase.from('organization_members').select('org_id, role, created_at, invite_accepted_at').eq('user_id', user.id),
    supabase.from('crew_members').select('id, name, email, phone, role, specialty, is_active, created_at').eq('user_id', user.id).maybeSingle(),
    supabase.from('push_subscriptions').select('endpoint, created_at').eq('user_id', user.id),
    supabase.from('audit_events').select('action, target_type, target_id, created_at').eq('actor_id', user.id).order('created_at', { ascending: false }).limit(500),
  ])

  const { data: turnoverAssignments } = crewProfile?.id
    ? await supabase.from('turnover_assignments').select('turnover_id, assigned_at, completed_at').eq('crew_member_id', crewProfile.id)
    : { data: [] }
  ...
  const payload = {
    ...
    turnover_assignments: turnoverAssignments ?? [],
    push_subscriptions:   (pushSubscriptions ?? []).map((s) => ({ endpoint: s.endpoint, created_at: s.created_at })),
