...
import type { AssetType, CrewLocale } from '@/types/database'
    ...
    <div>
      ...
      {missingTypes.length === 0 ? (
        <div className="bg-card-themed rounded-xl border border-themed p-6 text-center mb-4">
          ...
          <p className="text-sm text-muted-themed">{t('assetsEveryAssetDiscovered')}</p>
        </div>
      ) : (
          ...
          {missingTypes.map((assetType) => (
            ...
            >
              <span className="text-sm font-medium text-primary-themed">
                {assetTypeDisplayName(assetType, crewLocale)}
              </span>
              <span
...
function PlaceWorkOrderModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!title.trim()) { setError(t('woModalErrorDescribe')); return }
    setSubmitting(true)
    setError(null)
  ...
  return (
    <Dialog
      open
      onClose={onClose}
      title={success ? t('woModalTitlePlaced') : t('assetsPlaceWorkOrder')}
      ...
      footer={
        success ? (
          ...
          >
            {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> {t('actionSubmitting')}</> : <><Wrench className="w-4 h-4" /> {t('actionSubmit')}</>}
          </button>
        )
      }
    >
      {success ? (
        ...
        <>
          ...
          <form id="place-wo-form" onSubmit={handleSubmit} className="space-y-3">
            <div>
              <label htmlFor="wo-property" className="label text-primary-themed">{t('woModalPropertyLabel')}</label>
              <Input id="wo-property" type="text" value={propertyName} disabled />
            </div>
            <div>
              <label htmlFor="wo-asset" className="label text-primary-themed">{t('woModalAssetLabel')}</label>
              <select
                id="wo-asset"
                value={assetId}
                ...
                className="input"
              >
                <option value="">{t('woModalAssetOther')}</option>
                {discoveredAssets.map((a) => (
                  <option key={a.id} value={a.id}>{assetLabel(a, crewLocale)}</option>
                ))}
              </select>
            </div>
            <div>
              <label htmlFor="wo-issue" className="label text-primary-themed">{t('woModalIssueLabel')}</label>
              <Input
                id="wo-issue"
                type="text"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                placeholder={t('woModalIssuePlaceholder')}
                required
              />
            </div>
