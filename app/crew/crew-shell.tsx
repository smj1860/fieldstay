...
const FAQ_ITEMS: Array<{ q: Record<CrewLocale, string>; a: Record<CrewLocale, string> }> = [
  ...
  },
  {
    ...
    a: {
      en: 'The numbers you enter are used to fill an actual shopping cart or generate a purchase order to restock the property. An inaccurate count means either too much is ordered (waste) or not enough (the next guest arrives to find empty shelves). Your count is the direct input to that process.',
  ...
  },
  {
    ...
    a: {
      en: 'Keep the app open (or reopen it) once you’re back in coverage. It checks for a connection the moment you regain signal and again every 30 seconds while it’s open, but it doesn’t sync while fully closed in the background. If you finish a job with no signal, open the app again once you’re somewhere with service.',
  ...
  },
  {
    ...
    a: {
      en: 'The app automatically resizes and compresses every photo before saving it, so a full day of checklist and asset photos takes up far less space than the originals would. You don’t need to manage this yourself.',
