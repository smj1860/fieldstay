...
- Status: FIXED — re-verified against the current tree. `guidebook-sms-evening-cron.ts`, `guidebook-sms-morning-cron.ts`, `guidebook-pre-arrival-email-cron.ts`, `guidebook-stay-extension-cron.ts`, `cron/checklist-signals.ts`, and `cron/work-order-ops.ts` now instantiate `createServiceClient()` exclusively inside `step.run()` callbacks — no outer-scope instance remains in any of them. `ownerrez/initial-sync.ts`'s `writeSyncCount()` helper (line 48) creates a fresh client per call, same as `build-shopping-cart.ts`'s `persistCartStatus` pattern, and is itself only ever invoked from inside `step.run()` callbacks (lines 104, 441, 525) — not a stale-client-across-retries case. `inventory-order-email-cron.ts` no longer exists in the codebase — retired/superseded by the notification-digest cron, so it's moot rather than fixed. Residual: `build-shopping-cart.ts:128` still has one bare `createServiceClient()` at outer scope guarding an idempotent `org_milestones` upsert (`onConflict`, `ignoreDuplicates: true`) — see `audit/01-rls-security-multitenant.md` Finding 7 for the same note; low-priority style residual, not the original defect.
- Fix: None needed — already applied for all 9 originally-flagged files (one is now moot). Original fix recommendation retained below for historical record: move `const supabase = createServiceClient()` inside each individual `step.run()` callback in all 9 files, matching the corrected pattern in the round-1-fixed files (guidebook-sponsor-activated.ts, work-order-crew-assigned.ts, etc.). Recommend a follow-up grep-assisted manual sweep across the ENTIRE `lib/inngest/functions/` tree (including subdirectories) rather than spot-checking — this round found these 9 by mechanically comparing byte offsets of `createServiceClient()` calls against the first `step.run(` call in every file, which round 1's manual approach apparently missed.
