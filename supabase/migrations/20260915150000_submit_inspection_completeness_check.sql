-- ============================================================================
-- submit_inspection() certified any item list as a complete, insurance-grade
-- inspection — including a single answered item out of 50+.
--
-- The fill-screen/review-page architecture is built entirely around the claim
-- that "Sign-off is the ONE control that blocks a signed declaration over a
-- form with eleven blank items" — but that guarantee lived EXCLUSIVELY in the
-- browser (ReviewPage's Sign-off button disabled by a client-computed
-- `outstanding` list). fill-screen.tsx's signOff() itself never re-checks
-- findOutstanding() before building the payload; parseSubmitPayload() is
-- documented SHAPE validation only (`items.length > 0` is the entire
-- completeness bar); and this function inserted whatever rows it was handed
-- and unconditionally set completed_at. Any admin/manager credential that
-- reaches the route (a compromised session, a stale JS bundle after a bad
-- deploy, a direct request, or a future bug in findOutstanding) could
-- certify an inspection with one answer, indistinguishable in the database
-- from a fully walked one.
--
-- This is a FLOOR, not a byte-for-byte mirror of the client's
-- findOutstanding(): it cannot see conditional (show_when) children, a
-- repeat group's actual member count, or the property's live asset ledger
-- (repeat_per_asset/per_unit size from that, not from the snapshot), so it
-- does not attempt to validate those here — doing so risks a FALSE POSITIVE
-- that blocks a genuinely complete walk, which is worse than the gap this
-- closes. What it enforces is the set of items that are required on EVERY
-- walk of this form regardless of property or prior answers: root items
-- (no parent_item_id), not sized by a repeat count (no
-- repeat_source_item_id), not per-asset (repeat_per_asset/per_unit both
-- false), and not gated behind a property fact
-- (asks_property_fact/shown_when_property_fact both null). That is exactly
-- the class of item the described attack skips, and requiring it closes the
-- "one answer out of fifty" forgery without any risk of rejecting a
-- legitimate submission a conditional branch or an asset-count answers for.
-- ============================================================================
...
DECLARE
  ...
  v_missing       integer;
BEGIN
  SELECT org_id, property_id, completed_at, form_snapshot
    INTO v_org_id, v_property_id, v_completed, v_form_snapshot
