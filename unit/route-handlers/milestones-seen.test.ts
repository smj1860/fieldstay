...
import { POST } from '@/app/api/milestones/seen/route'
...
function postRequest(body: unknown) {
  return new NextRequest('http://localhost/api/milestones/seen', {
    method:  'POST',
...
describe('POST /api/milestones/seen', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.useFakeTimers()
  ...
  it('rejects an unparseable body without calling requireOrgMember', async () => {
    const req = new NextRequest('http://localhost/api/milestones/seen', {
      method:  'POST',
      headers: { 'content-type': 'application/json' },
      body:    'not-json',
    })
  ...
  it('propagates the auth failure (e.g. redirect-to-login) instead of swallowing it', async () => {
    ...
    await expect(POST(postRequest({ milestone: 'inventory_setup' }))).rejects.toThrow('REDIRECT:/login')
  })

  it('stamps prompted_at, scoped to the caller org_id, on the happy path', async () => {
    const supabase = makeSupabase()
    mockAuthed(supabase)

    const res = await POST(postRequest({ milestone: 'inventory_setup' }))
    ...
    expect(supabase.calls[0]).toEqual({
      method: 'update',
      args:   [{ prompted_at: FIXED_NOW }],
    })
    expect(supabase.calls[1]).toEqual({ method: 'eq', args: ['org_id', ORG_ID] })
    expect(supabase.calls[2]).toEqual({ method: 'eq', args: ['milestone', 'inventory_setup'] })
  })
  ...
  it('IDOR: ignores a client-supplied org_id and scopes the update to the session org_id only', async () => {
    ...
    const res = await POST(
      postRequest({ milestone: 'inventory_setup', org_id: 'attacker_org' }),
    )
