...
const EXCEPTIONS: Record<string, string> = {
  ...
  'lib/inngest/functions/cron/work-order-ops.ts:170':
  ...
  'lib/inngest/functions/cron/maintenance-schedules.ts:326':
    'REAL GAP, not fixed — the work_order_updates escalate-to-urgent-plus-note pattern in the per-org overdue pass. Unlike cron/work-order-ops.ts:170 (now guarded by an optimistic-locked bulk update), this branch updates a single WO without a returning-rows precondition, so a step retry can still append a duplicate note. Cosmetic impact only (a duplicate note, not a duplicate financial/state record); closing it needs a product decision about what makes two escalation events "the same".',
}
