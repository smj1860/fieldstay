...
export function DexieProvider({ userId: userIdProp, children }: { userId?: string; children: ReactNode }) {
  ...
  if (assignError) {
    ...
    return  // do NOT advance watermark on error
  }
  ...
  if (!turnoverIds.length) {
    ...
    // An incremental pull with zero results is a no-op — don't move the cursor.
    if (!since) {
  ...
  if (tErr) {
    ...
    return  // do NOT advance watermark — next sync will retry
  }
  ...
  // force=true: full pull on mount, clears any stale watermark
  ...
  channel = supabase
    ...
    .on(
      ...
      () => { void syncAssignedTurnovers(crewMember.id, false) }  // incremental for RT events
    )
    .subscribe()
}
