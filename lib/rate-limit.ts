...
// rather than IP: this endpoint accepts client-asserted incident data (the
// implementation doc's section 3.3), so the limiter checked here bounds an
// authenticated identity spamming batches, not an anonymous scan. 20/minute
// comfortably covers a device flushing its whole unreported queue on every
// reconnect (batches of up to 50) without opening a spend/DoS vector.
