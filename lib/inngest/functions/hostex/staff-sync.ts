...
export async function syncHostexStaff(
  ...
  const crewCount = await step.run(`${stepPrefix}-upsert-crew`, async () => {
    ...
    // ROLE IS OURS, NOT HOSTEX'S — written once at insert and never again,
    // unlike every other column here, which Hostex genuinely reports and which
    // should be overwritten. Hostex staff records carry no role field at all;
    // the role in `rows` is INFERRED from each person's scheduled task types,
    // and this step runs daily, so writing the inference unconditionally
    // reverted a PM's correction every morning.
    //
    // Why the stored value wins unconditionally, including 'general', and what
    // that costs: see preserve-crew-roles.ts. Kept in one place because the
    // reasoning is identical for Hospitable, which had the same bug for ten
    // days longer precisely because this logic lived here instead.
    const preserved = await preserveManualCrewRoles(
      supabase, orgId, PROVIDER, rows, 'inngest.hostex-staff-sync.existing-roles',
    )
