...
// and OwnerRezApiClient's shared fetch transport (used by every OwnerRez
// consumer EXCEPT incremental-sync — initial-sync, reviews-sync and
// reconciliation-handler have no breaker of any kind today) genuinely had
// none. 'hospitable' and 'ownerrez' were added to BreakerProvider and wired
// into hospitableFetch / OwnerRezApiClient.fetchUrl for exactly that reason —
// as a platform-wide, transport-level backstop that runs ALONGSIDE OwnerRez's
// existing per-connection one, not a replacement for it. Mapbox and
// Tomorrow.io are unaddressed follow-up, out of scope for this pass.
//
...
export type BreakerProvider = 'kroger' | 'telnyx' | 'mapbox' | 'tomorrow' | 'hospitable' | 'ownerrez'
